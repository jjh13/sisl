...
void print_board(const sisl::array_n<bool, 2> &board, const int &m, const int &n) {
	for(uint i = 0; i < m; i++)
		for(uint j = 0; j < n; j++) {
			print_at(
				i,j,
...
uint count_neighbours(const sisl::array_n<bool, 2> &board, const int &m, const int &n, const int &i, const int &j) {
	uint count = 0;
...
sisl::array_n<bool, 2> advance_board(const sisl::array_n<bool, 2> &board, const int &m, const int &n) {
	sisl::array_n<bool, 2> new_board(m,n);
...
int main(int argc, char *argv[]) {
	...
	sisl::array_n<bool, 2> board(30, 60);
